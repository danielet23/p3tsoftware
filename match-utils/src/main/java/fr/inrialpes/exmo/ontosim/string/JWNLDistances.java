...
public class JWNLDistances {
	final static Logger logger = LoggerFactory.getLogger(JWNLDistances.class);
	...
	if ( iws != null ) {
	    // not iterable...
		Collection<IndexWord> collection = iws.getIndexWordCollection();
	 ...
	 * @param Syno2
	 */
    private void computeTokenSimilarityInnerA(IndexWord index2, PointerTargetNodeList ptnl1, PointerTargetNodeList ptnl2, int maxBetweenLists, double maxCommon, PointerTargetNodeList best1, PointerTargetNodeList best2, Synset[] Syno1, Synset synset1, Synset[] Syno2) throws JWNLException {
        ...
        if (index1 != null && index2 != null) {
            ...
            try {
                ...
                for (int i = 0; i < senseCount1; i++) {
                    ...
                    Synset synset1 = Syno1[i];
                    int senseCount2 = 1;//hasSenseCount();
    ...
    public int getCommonConcepts(PointerTargetNodeList list1,
		...
		Synset pointerTargetNode1 = ((PointerTargetNode) list1.get(list1.size() - i)).getSynset();
		Synset pointerTargetNode2 = ((PointerTargetNode) list2.get(list2.size() - i)).getSynset();
