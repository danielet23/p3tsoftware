...
public abstract class ASAbstractCoverageTraversal extends AbstractAlignmentSpaceMeasure<LoadedOntology<?>> {
	final static Logger logger = LoggerFactory.getLogger(ASAbstractCoverageTraversal.class);
	...
	public Hashtable<URI, URI> traverse(Hashtable<URI, URI> max, int size, Hashtable<URI, URI> topreserve, URI current,
			URI target) {
		if (current.equals(target)) { // Stop if finished
			...
			return traverseAlreadyVisited(max, size, topreserve, current, target);
		} else {
			return max; // ??
		}
	}
	
	public Hashtable<URI, URI> traverseAlreadyVisited(Hashtable<URI, URI> max, int size, Hashtable<URI, URI> topreserve, URI current,
			URI target) {
			if (prevtable2.get(current) == null)
				prevtable2.put(current, new HashSet<Hashtable<URI, URI>>());
			...
			for (Alignment al : network.getSourceAlignments(current)) {
				...
				try {
					if (preserved.size() > result.size()) { // we can try to do
															// better
						// is it really preserved.size() ??
						Hashtable<URI, URI> obtained = traverse(result, preserved.size(), preserved,
								al.getOntology2URI(), target);
						if (obtained.size() == size)
							return obtained; // we reached the end with optimal
												// preservation
						else if (obtained.size() > result.size())
							result = obtained;
					}
	...
	public Hashtable<URI, URI> applyAlignment(Alignment al, Hashtable<URI, URI> entities) {
		...
		return fillResults(align, entities);
	}
	
	private Hashtable<URI, URI> fillResults(URIAlignment align, Hashtable<URI, URI> entities) {
     ...
     */
    public Set<URI> objectsToPreserve( Ontology o1 ) throws OntoSimException, OntowrapException {
