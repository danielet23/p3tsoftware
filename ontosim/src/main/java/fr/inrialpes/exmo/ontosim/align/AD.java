...
public abstract class AD extends AbstractAlignmentSpaceMeasure<HeavyLoadedOntology<OntModel>> {
	final static Logger logger = LoggerFactory.getLogger(AD.class);
	...
	protected String[] getRModules(String s, String predFull, String o, HeavyLoadedOntology<OntModel> onto1)
		...
		if (checkOWLlanguage(p)) {
			/* skip it */ return new String[0];
		}
		...
		fillResult(onto1, result, s, "matching", "");
		
		// JD: switch s and o
		fillResult(onto1, result, o, "matching-1", "-1");

		if (onto.getOntResource(s).isClass()) {
			resourceIsClass(s, o, onto1, result);
		}
		...
		if (onto.getOntResource(s).isProperty()) {
			resourceIsProperty(s, o, onto1, result);
		}

		// TODO eliminate redundancy
		// minimalize:
		eliminateRedundancy(result);
	...
	private void resourceIsClass(String s, String o, HeavyLoadedOntology<OntModel> onto1, HashSet<String> result){
		OntModel onto = onto1.getOntology();
		OntClass c = onto.getOntResource(s).asClass();
		if (!result.contains("subClassOf")) {

			String[] scos = getUris(onto1.getSuperClasses(c, OntologyFactory.LOCAL, OntologyFactory.INDIRECT,
					OntologyFactory.NAMED), onto1);
			if (contains(scos, o))
				result.add("subClassOf");
		}
		if (!result.contains("subClassOf-1")) {
			String[] scos = getUris(
					onto1.getSubClasses(c, OntologyFactory.LOCAL, OntologyFactory.INDIRECT, OntologyFactory.NAMED),
					onto1);
			if (contains(scos, o))
				result.add("subClassOf-1");
		}
		if (!result.contains("disjointWith")) {
			...
			if (contains(scos, o))
				result.add("disjointWith");
		}
	}
	
	private void resourceIsIndividual(String s, String o, HeavyLoadedOntology<OntModel> onto1, HashSet<String> result){
		if (!result.contains("type")) {
			String[] scos = getUris(onto1.getClasses(onto1.getEntity(URI.create(s)), OntologyFactory.LOCAL,
					OntologyFactory.INDIRECT, OntologyFactory.NAMED), onto1);
			if (contains(scos, o))
				result.add("type");
		}
		if (!result.contains("type-1")) {
			String[] scos = getUris(onto1.getInstances(onto1.getEntity(URI.create(s)), OntologyFactory.LOCAL,
					OntologyFactory.INDIRECT, OntologyFactory.NAMED), onto1);
			if (contains(scos, o))
				result.add("type-1");
		}
	...
	private void resourceIsProperty(String s, String o, HeavyLoadedOntology<OntModel> onto1, HashSet<String> result){
		OntModel onto = onto1.getOntology();
		...
		/*
		 * if (!result.contains("domain-1")){ String[] scos =
		 * getUris(onto1.getDomain(prop, OntologyFactory.INDIRECT),onto1);
		...
		/*
		 * if (!result.contains("range-1")){ String[] scos =
		 * getUris(onto1.getRange(prop, OntologyFactory.INDIRECT),onto1); if
		 * (contains(scos, o)) result.add("range-1"); }
		 */
		resultDoesntContainsSubProperty(s, o, onto1, result, "subPropertyOf");
		resultDoesntContainsSubProperty(s, o, onto1, result, "subPropertyOf-1");
	}
	
	private void resultDoesntContainsSubProperty(String s, String o, HeavyLoadedOntology<OntModel> onto1, HashSet<String> result, String subProperty){
		OntModel onto = onto1.getOntology();
		OntProperty prop = onto.getOntResource(s).asProperty();
		if (!result.contains(subProperty)) {
			String[] scos = getUris(onto1.getSuperProperties(prop, OntologyFactory.LOCAL, OntologyFactory.INDIRECT,
					OntologyFactory.NAMED), onto1);
			if (contains(scos, o))
	...
	private void eliminateRedundancy(HashSet<String> result){
	...
	private boolean checkOWLlanguage(String p){
		boolean flag = false;
		if(!contains(lrelations, p) && (predFull.startsWith(OWL.NS) || predFull.startsWith(OWL2.NS))){
				flag=true;
		}
		return flag;
	}	
	
	private boolean fillResult(HeavyLoadedOntology<OntModel> onto1, HashSet<String> result, String s, String matching, String n){
		OntModel onto = onto1.getOntology();
		...
		while (it.hasNext()) {
			...
			if (contains(lrelations, rel) && !result.contains(rel+n)) {
				result.add(rel+n);
	...
	public double disagreement(HeavyLoadedOntology<OntModel> onto1, HeavyLoadedOntology<OntModel> onto2) {
		// System.out.println("--- DISAGREEMENT BETWEEN : "+onto1.getURI()+" and
		// "+onto2.getURI());
		try {
		
			Container cont1 = new Container(0,0);
			Container cont2 = new Container(0,0);
			
			getSumAndCountDisagreement(onto1, onto2, cont1);
			getSumAndCountDisagreement(onto2, onto1, cont2);
			
			if (cont1.count == 0 && cont2.count == 0)
				return 0;
			return (cont1.sum + cont2.sum) / (((double) cont1.count) + ((double) cont2.count));
		} catch (OntowrapException e) {
	...
	private void getSumAndCountDisagreement(HeavyLoadedOntology<OntModel> onto1, HeavyLoadedOntology<OntModel> onto2, Container cont) throws OntowrapException, AlignmentException{
			Vector<String[]> ST1 = listStatements(onto1);
			for (String[] st : ST1) {
				// if (correspondences.get(st[0])!=null)
				for (Cell cs : getImages(st[0], onto1, onto2)) {// (String s :
																// correspondences.get(st[0]))
																// {
					...
					// if (onto2.getOntology().getOntResource(s)!=null &&
					// correspondences.get(st[2])!=null)
					for (Cell co : getImages(st[2], onto1, onto2)) {// for
																	// (String p
																	// :
																	// correspondences.get(st[2]))
																	// {
						...
						if (RM.length != 0) {
							...
							// System.out.println("Disgreement "+ag+" for onto2
							// on "+st[0]+"-"+st[1]+"-"+st[2]);
	...
	private class Container{
		...
		public Container(double sum, int count){
				this.sum = sum;
				this.count = count;
		}
	}
	...
	public double agreement(HeavyLoadedOntology<OntModel> onto1, HeavyLoadedOntology<OntModel> onto2) {
		// System.out.println("--- AGREEMENT BETWEEN : "+onto1.getURI()+" and
		// "+onto2.getURI());
		try {
			...
			getSumAndCountAgreement(onto1, onto2, cont1);
			getSumAndCountAgreement(onto2, onto1, cont2);
	...
	private void getSumAndCountAgreement(HeavyLoadedOntology<OntModel> onto1, HeavyLoadedOntology<OntModel> onto2, Container cont) throws OntowrapException, AlignmentException{
			Vector<String[]> ST1 = listStatements(onto1);
			for (String[] st : ST1) {
				// if (correspondences.get(st[0])!=null)
				for (Cell cs : getImages(st[0], onto1, onto2)) {// (String s :
					...
					for (Cell co : getImages(st[2], onto1, onto2)) {// for
						...
						if (RM.length != 0) {
							...
							// System.out.println("Agreement "+ag+" for onto2 on
							// "+st[0]+"-"+st[1]+"-"+st[2]);
							cont.sum += ag;
							cont.count++;
						}
    ...
    public static void testAD(String[] args) {
