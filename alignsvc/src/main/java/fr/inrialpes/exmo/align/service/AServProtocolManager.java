...
public class AServProtocolManager {
   ...
   /*********************************************************************
    ...
    public Message load( Message mess ) {
		...
		String file = null;
		if ( isStringVoid(name) ){
			file  = params.getProperty("filename");
			if ( isStringVoid(file) ) name = "file://"+file;
		}
	...
	private boolean isStringVoid(String s){
    ...
    public Message findCorrespondences( Message mess ) {
		...
		try {
			...
			return new UnknownAlignment(newId(),mess,serverId,mess.getSender(),id,p);
		}
		// Find matched
		URI uri = null;
		try {
			...
			return new ErrorMsg(newId(),mess,serverId,mess.getSender(),"MalformedURI problem",p);
		};
		// Retrieve correspondences
		String msg = params.getProperty("strict");
		boolean strict = getStrict(msg);
		msg = "";
		try {
			...
			return new ErrorMsg(newId(),mess,serverId,mess.getSender(),"Unexpected Alignment API Error",p);
		}
		return new EntityList( newId(), mess, serverId, mess.getSender(), msg, p );
    }
	
	private boolean getStrict(String msg){
		return ( msg != null && !msg.equals("0") && !msg.equals("false") && !msg.equals("no") );
	}
	...
	try {
	    ...
	    return new TranslatedMessage(newId(),mess,serverId,mess.getSender(),translation, p);
	} catch (AlignmentException e) {
	    return new ErrorMsg(newId(),mess,serverId,mess.getSender(),e.toString(), p);
	}
    }
	...
	try { 
		...
		return new UnknownMethod(newId(),mess,serverId,mess.getSender(),method, p);
	    }
	    renderer.init( params );
	    al.render( renderer );
	} catch ( AlignmentException e ) {
	    return new CannotRenderAlignment(newId(),mess,serverId,mess.getSender(),id, p);
	    ...
	    return new Message(newId(),mess,serverId,mess.getSender(),"Failed to render alignment", p);
	...
	return new RenderedAlignment(newId(),mess,serverId,mess.getSender(),result.toString(), p);
    }
	...
	return new AlignmentId(newId(),mess,serverId,mess.getSender(),newId, p,
			       al.getExtension( Namespace.ALIGNMENT.uri, Annotations.PRETTY ));
    }

    public Message harden( Message mess ){
	return new AlignmentId(newId(),mess,serverId,mess.getSender(),"Harden not implemented", p);
    }
	...
	try {
	    ...
	    return new UnknownAlignment(newId(),mess,serverId,mess.getSender(),"unknown/Alignment/"+id, p);
	}
	...
	public Message eval( Message mess ){
		...
		try {
			...
			return new UnknownAlignment(newId(),mess,serverId,mess.getSender(),"unknown/Alignment/"+msg, p);
		}
		...
		try {
			...
			logger.error( "Unknown method", cnfex );
			return new UnknownMethod(newId(),mess,serverId,mess.getSender(),classname, p);
		} catch ( InvocationTargetException itex ) {
			msg = itex.toString();
			if ( itex.getCause() != null ) msg = itex.getCause().toString();
			return new ErrorMsg(newId(),mess,serverId,mess.getSender(),msg, p);
		} catch ( Exception ex ) {
			return new ErrorMsg(newId(),mess,serverId,mess.getSender(),ex.toString(), p);
		}
		// Compare it
	...
	try {
	    ...
	    return new UnknownAlignment(newId(),mess,serverId,mess.getSender(),"unknown/Alignment/"+id1, p);
	}
	// Retrieve the reference alignment
	...
	try {
	    ...
	    return new UnknownAlignment(newId(),mess,serverId,mess.getSender(),"unknown/Alignment/"+id2, p);
	}
	try { 
	    DiffEvaluator diff = new DiffEvaluator( al1, al2 );
	    diff.eval( params ); 
	    // This will only work with HTML
	    return new EvalResult(newId(),mess,serverId,mess.getSender(),diff.HTMLString(), p);
	} catch (AlignmentException e) {
	    return new ErrorMsg(newId(),mess,serverId,mess.getSender(),e.toString(), p);
	}
    }
		...
		while ( tk != null && tk.hasMoreTokens() ){
			...
			try {
				...
				if ( file.isDirectory() ) {
					...
					getClassList(list, file, tosubclass);
				} else if ( checkFile(file, visited)) {
					//logger.trace("JAR {}", file);
					classPath = getClassPath(visited, file, tosubclass, list);
				}
			} catch( IOException e ) {
				continue;
			}
	...
	private static boolean checkFile(File file, Set<String> visited){
		boolean flag = false;
		if(file.toString().endsWith(".jar") && file.exists() && !visited.contains( file.toString() )){
			flag = true;
		}
		return flag;
	}
	
	private static void getClassList(Set<String> list, File file, Class tosubclass) throws IOException{
	...
	private static String getClassPath(Set<String> visited, File file, Class tosubclass, Set<String> list ){
	...
	while( enumeration != null && enumeration.hasMoreElements() ){
		...
		JarFile inJar = openJar(visited, jar, entry);
	...
	private static JarFile openJar(Set<String> visited, JarFile jar, JarEntry entry ){
	...
	public void run() {
	    ...
	    Alignment init = findInitialAlignment();

		if(init==null){
			return;
		}
	    // Create alignment object
		checkAlignmentObject(uri1, uri2, init);
	}
	
	private boolean getURI(URI uri1, URI uri2){
		boolean flag = true;
		Properties params = mess.getParameters();
		try {
			uri1 = new URI(params.getProperty("onto1"));
	...
	private Alignment findInitialAlignment(){
		Alignment init = null;
		Properties params = mess.getParameters();
	...
	private void checkAlignmentObject(URI uri1, URI uri2, Alignment init){
		Properties p = (Properties) null;
		Properties params = mess.getParameters();
		String method = params.getProperty("method");
		try {

			createAlignmentObject(uri1, uri2, init, params, method);
	    } catch ( ClassNotFoundException cnfex ) {
		logger.error( "Unknown method", cnfex );
		result = new UnknownMethod(newId(),mess,serverId,mess.getSender(),method, p);
	    } catch (NoSuchMethodException e) {
		result = new RunTimeError(newId(),mess,serverId,mess.getSender(),"No such method: "+method+"(Object, Object)", p);
	    } catch (InstantiationException e) {
		result = new RunTimeError(newId(),mess,serverId,mess.getSender(),"Instantiation", p);
	    } catch (IllegalAccessException e) {
		result = new RunTimeError(newId(),mess,serverId,mess.getSender(),"Cannot access", p);
	    } catch (InvocationTargetException e) {
		result = new RunTimeError(newId(),mess,serverId,mess.getSender(),"Invocation target", p);
	    } catch (AlignmentException e) {
		result = new NonConformParameters(newId(),mess,serverId,mess.getSender(),"nonconform/params/", p);
	    } catch (Exception e) {
		result = new RunTimeError(newId(),mess,serverId,mess.getSender(),"Unexpected exception :"+e, p);
	    }
	}
	
	private void createAlignmentObject(URI uri1, URI uri2, Alignment init, Properties params, String method ) throws ClassNotFoundException, NoSuchMethodException, InstantiationException,
						IllegalAccessException, InvocationTargetException, AlignmentException, Exception{
		...
		Properties p = (Properties) null;
			...
			try {
				...
				result = new UnreachableOntology(newId(),mess,serverId,mess.getSender(),params.getProperty("onto1"), p);
				} else if ( reachable( uri2 ) == null ){
				result = new UnreachableOntology(newId(),mess,serverId,mess.getSender(),params.getProperty("onto2"), p);
				} else {
				result = new NonConformParameters(newId(),mess,serverId,mess.getSender(),"nonconform/params/"+e.getMessage(), p);
				}
				return;
			}
			// ask to store A'
			alignmentCache.recordNewAlignment( id, aresult, true );
			result = new AlignmentId(newId(),mess,serverId,mess.getSender(),id, p,
