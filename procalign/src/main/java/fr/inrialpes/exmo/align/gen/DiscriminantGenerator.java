...
public class DiscriminantGenerator extends TestSet {
	...
	initTests( "000" );
		this.initTestCasesInner( STEP, INCR);
    }
	 ...
	 * @param INCR
	 */
	private void initTestCasesInner(int STEP, final float INCR){
		for ( int i = 0; i < STEP; i++ ) {
			...
			this.initTestCasesInnerInner(STEP, INCR, label1, i);
		}
	}
	 ...
	 * @param i
	 */
	private void initTestCasesInnerInner(int STEP, final float INCR, String label1, int i){
		for ( int j = 0; j < STEP; j++ ) {
			...
			this.initTestCasesInnerInnerInner(STEP, INCR, label2, i, j);
		}
	}
	 ...
	 * @param j
	 */
	private void initTestCasesInnerInnerInner(int STEP, final float INCR, String label2, int i, int j){
		for ( int k = 0; k < STEP; k++ ) {
			String label3 = ""+i+j+k;
			if ( i != 0 || j != 0 || k != 0 ) {
				addTestChild( label2, label3,
