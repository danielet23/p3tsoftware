...
public class GroupEval {
	...
	private class CaseF implements Handler {
		public void execute() {
			format = g.getOptarg();
		}
	}
	...
	private class CaseS implements Handler {
		public void execute() {
	...
	private class CaseL implements Handler {
		public void execute() {
	...
	private class CaseW implements Handler {
		public void execute() {
			arg = g.getOptarg();
    ...
    public static void printHTMLandLATEXMethodA(String format, Vector<String> listAlgo, int fsize, PrintStream writer) {
		for ( String m : listAlgo ) {
	...
	public static void printHTMLandLATEXMethodB(String format, PrintStream writer, int fsize, PRecEvaluator eval, Formatter formatter) {
		for ( int i = 0 ; i < fsize; i++){
			writer.print("<td>");
			if ( format.charAt(i) == 'p' ) {
	...
	public static void printHTMLandLATEXMethodC(String format, int i, Formatter formatter, PrintStream writer, double precision, double recall, long[] timeVect, int k) {
		if ( format.charAt(i) == 'p' ) {
	...
	public static void printHTMLMethodAInner(int nexpected, int expected, int[] foundVect, int[] correctVect, long[] timeVect, int k, String format,
										PrintStream writer, int fsize, Formatter formatter, Enumeration f) {
		PRecEvaluator eval = (PRecEvaluator)f.nextElement();
		if ( eval != null ){
			// iterative H-means computation
			if ( nexpected == -1 ){
			...
			printHTMLandLATEXMethodB(format, writer, fsize, eval, formatter);
	...
	public static void printHTMLMethodA(boolean colored, Vector<Vector> result, String color, PrintStream writer, int expected,
										int[] foundVect, int[] correctVect, long[] timeVect, String format, int fsize, Formatter formatter) {
		for ( Vector test : result ) {
			int nexpected = -1;
			...
			f.nextElement();
			for( int k = 0 ; f.hasMoreElements() ; k++) {
				printHTMLMethodAInner(nexpected, expected, foundVect, correctVect, timeVect, k, format, writer, fsize, formatter, f);
			}
			writer.println("</tr>");
		}
	}
	...
	printHTMLandLATEXMethodA(format, listAlgo, fsize, writer);
	...
	printHTMLMethodA(colored, result, color, writer, expected, foundVect, correctVect, timeVect, format, fsize, formatter);
	...
	for ( String m : listAlgo ) {
	    ...
	    for ( int i = 0 ; i < fsize; i++){
		writer.print("<td>");
		printHTMLandLATEXMethodC(format, i, formatter, writer, precision, recall, timeVect, k);
		writer.println("</td>");
	    };
	    k++;
	}
    ...
    public static void printLATEXMethodA(int nexpected, int expected, int[] foundVect, int[] correctVect, long[] timeVect, String format,
										 PrintStream writer, int fsize, Formatter formatter, Enumeration f) {
		for( int k = 0 ; f.hasMoreElements() ; k++) {
			PRecEvaluator eval = (PRecEvaluator)f.nextElement();
			if ( eval != null ){
	...
	for ( Vector test : result ) {
	    ...
	    Enumeration f = test.elements();
	    f.nextElement();
	    printLATEXMethodA(nexpected, expected, foundVect, correctVect, timeVect, format, writer, fsize, formatter, f);
	    writer.println(" \\\\");
	}
	writer.print("H-mean");
	// Here we are computing a sheer average.
