...
public class BasicAlignment implements Alignment {
	...
	Set<Cell> s1 = hash1.get(c.getObject1());
		this.addCellInnerA(s1, c, found);

	found = false;
	Set<Cell> s2 = hash2.get(c.getObject2());

	this.addCellInnerB(s2, c, found);
	 ...
	 * @param found
	 */
	private void addCellInnerA(Set<Cell> s1, Cell c, boolean found){
		if ( s1 != null ) {
			// I must check that there is no one here
			this.addCellInnerAFor(s1, found, c);
			if (!found) s1.add( c );
		} else {
			s1 = new HashSet<Cell>();
			s1.add( c );
	 ...
	 * @param c
	 */
	private void addCellInnerAFor(Set<Cell> s1, boolean found, Cell c){
	 ...
	 * @param found
	 */
	private void addCellInnerB(Set<Cell> s2, Cell c, boolean found){
		if( s2 != null ){
			// I must check that there is no one here
			this.addCellInnerBFor(s2, found, c);
			if (!found)	s2.add( c );
		} else {
			s2 = new HashSet<Cell>();
			s2.add( c );
	 ...
	 * @param c
	 */
	private void addCellInnerBFor(Set<Cell> s2, boolean found, Cell c){
