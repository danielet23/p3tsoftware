...
public class PRGraphEvaluator extends GraphEvaluator {
	...
	double nbcorrect = 0;
	double nbfound = 0;
	double increment = (STEP*nbexpected)/100; // 2010 should be computed with the total expected (in negrapheval)
	...
	for ( EvalCell c : cellSet ) {
	    ...
	    if ( c.correct() ) {
		nbcorrect++;
		double precision = nbcorrect / nbfound;
		sumprecisions += precision; // For MAP
		if ( nbcorrect == next ) { // increment achieved
		    //record value
		    double recall = nbcorrect / nbexpected;
	...
	points.add( new Pair( nbcorrect / nbexpected, nbcorrect / nbfound ) );
	...
	double level = i/STEP; // max level of that interval
	...
	for( i = 0; i <= STEP; i++ ) {
	    ...
	    points.add( new Pair( i/10, precisions[i] ) );
	}
