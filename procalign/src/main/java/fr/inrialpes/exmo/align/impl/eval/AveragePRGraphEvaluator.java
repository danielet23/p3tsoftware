...
public class AveragePRGraphEvaluator extends GraphEvaluator {
	...
	if ( align2 == null ) return; //no increase of precisions
		...
		this.evalAlignmentInnerA(align1, align2);
	...
	inflexion.add( new Pair( 0., 1. ) ); // [R=0%]
		...
		this.evalAlignmentInnerB(nbfound, nbcorrect, sumprecisions, inflexion);
	...
	double best = 0.; // best value found for that interval
		...
		this.evalAlignmentInnerC(j,inflexion, level, i, best);
	 ...
	 * @param align2
	 */
	private void evalAlignmentInnerA(Alignment align1, Alignment align2){
		for ( Cell c : align2 ) {
	 ...
	 * @param inflexion
	 */
	private void evalAlignmentInnerB(int nbfound, int nbcorrect, double sumprecisions, Vector<Pair> inflexion){
