...
public class GenPlot {
	...
	this.runForMethodC(toplot);
	// Set output file
	OutputStream stream = null;
	this.runIfElseMethodD( stream);
	...
	// Vector<Pair> -> .
		this.runIfElseMethodE(toplot);
    }

	/**
	 * New Method()
	 */
	private void runForMethodA(){
	 ...
	 * @param max
	 */
	private void runForMethodB(int max){
	 ...
	 */
	private void runForMethodC(Vector<Vector<Pair>> toplot) throws AlignmentException {
	 ...
	 * @param stream
	 */
	private void runIfElseMethodD(OutputStream stream){
		if (outFile == null) {
			stream = System.out;
		} else{
			this.runIfElseMethodDInner(stream);
		}
	}
	 ...
	 * @param toplot
	 */
	private void runIfElseMethodE(Vector<Vector<Pair>> toplot){
		if ( type.equals("tsv") ){
			printTSV( toplot );
		} else{
			this.runIfElseMethodF(toplot);
		}
	}
	 ...
	 * @param toplot
	 */
	private void runIfElseMethodF(Vector<Vector<Pair>> toplot){
		if ( type.equals("html") ) {
			printHTMLGGraph( toplot );
		} else{
			this.runIfElseMethodG(toplot);
		}
	}
	 ...
	 * @param toplot
	 */
	private void runIfElseMethodG(Vector<Vector<Pair>> toplot){
		if ( type.equals("tex") ) {
			printPGFTex( toplot );
		} else{
			System.err.println("Flag -t "+type+" : not implemented yet");
		}
	}
    ...
    private void runSwitch(int c, Getopt g, String arg){
		...
		/* Debug level  */
				arg = g.getOptarg();
				this.runSwitchIfElseA(arg);
				break;
			case 'D' :
		/* Parameter definition */
				arg = g.getOptarg();
				int index = arg.indexOf('=');
				this.runSwitchIfElseB(index, arg, g);
				break;
		}
	}
	 ...
	 * @param arg
	 */
	private void runSwitchIfElseA(String arg){
		if ( arg != null ) {
	 ...
	 * @param g
	 */
	private void runSwitchIfElseB(int index, String arg, Getopt g){
		if ( index != -1 ) {
	...
	this.printPGFTexForA(i, colortable, marktable);

	// And a legend
	output.println("% Legend");
	i = 0;

	this.printPGFTexForB(i, colortable, marktable);
	 ...
	 * @param marktable
	 */
    private void printPGFTexForA(int i, String colortable[], String marktable[]){
	 ...
	 * @param marktable
	 */
	private void printPGFTexForB(int i, String colortable[], String marktable[]){
		for ( String m : listAlgo ) {
			output.print("\\draw["+colortable[i%6] );
			if ( !listEvaluators.get(i).isValid() ) output.print(",dotted");
	...
	private void printPGFTexForC(int i, Vector<Vector<Pair>> result ){
	 ...
	 * @param writer
	 */
	private void printPGFTexForCInner(Vector<Pair> table, PrintWriter writer){
		for( Pair p : table ) {
