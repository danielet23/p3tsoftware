...
public class ROCCurveEvaluator extends GraphEvaluator {
    ...
    public void forCicleCellSet(int nbfound, int y, int x,int area, Pair last){
	...
	public void addingMax(int nbfound, int area, int y, int x, int scale){
		if ( nbfound != 0 ) { // or x != 0
	...
	public void scale(int scale, int x){
        for ( Pair p : points ){
            if ( scale != 0 ){
                p.setX( p.getX() / scale );
	...
	Pair last = new Pair( 0., 0. ); // [Origin]
		...
		this.forCicleCellSet(nbfound,y,x,area,last);
	...
	this.addingMax(nbfound,area,y,x,scale);
