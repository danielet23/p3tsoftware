...
public class BenchmarkGenerator extends TestSet {
	...
	this.initTestCasesInner(max, maximum, mod, incr);
	...
	this.initTestCasesInnerA(i1, maximum, SUFFIX, PREVTEST, multModality, incr);
        /* 203 *** no comments */
	//Too easy
	 ...
	 * @param incr
	 */
    private void initTestCasesInner(String max, int maximum, String mod, float incr){
	 ...
	 * @param incr
	 */
    private void initTestCasesInnerA(float i1, int maximum, String SUFFIX, String PREVTEST, boolean multModality, float incr){
		for ( int i = 0; i1 < 1.00f && i < maximum ; i++ ) { //
			...
			this.initTestCasesInnerAInner(i1, i, SUFFIX);
	 ...
	 * @param SUFFIX
	 */
	private void initTestCasesInnerAInner(float i1, int i, String SUFFIX){
		if ( i1 < 1.0f ) {
